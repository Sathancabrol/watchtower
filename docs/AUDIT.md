...
| 2 | **« la vue HQ n'est plus là »** | ✅ **résolu** | Tu l'as précisé : **« HQ c'est la watchtower dans le ciel »**. Ce n'était donc pas une régression mais une **absence** : la tour n'existait plus dans le code. **Recréée** (`src/hq.js`) — tour procédurale (aucun fichier 3D) qui **apparaît par paliers** comme les repères Maps : 🔦 balise ≤ 120 km → 🗼 silhouette ≤ 40 km → 🏰 détail ≤ 12 km. Elle suit « ME LOCALISER » et possède un bouton 🏰 HQ dans le lanceur. |
