...
/**
 * WATCHTOWER — keyless geocoding fallback.
 *
 * FREE mode has no Google key, and the upstream app answered every location
 * search with a thrown "No Google Maps API key available for geocoding".
 * These helpers geocode through keyless OSM services instead — Photon
 * (photon.komoot.io, CORS-enabled, typo-tolerant) first, Nominatim as the
 * backup — and shape the answer like a Google Geocoding result so the whole
 * fly-to pipeline below (navigation modes, viewport framing) runs unchanged.
 */

/** Map a Photon/Nominatim OSM place descriptor onto Google geocode types. */
export function osmPlaceToGoogleTypes(osmKey, osmValue) {
  const key = String(osmKey || '').toLowerCase();
  const value = String(osmValue || '').toLowerCase();
  if (value === 'country') return ['country'];
  if (['state', 'region', 'province'].includes(value)) return ['administrative_area_level_1'];
  if (['county', 'district', 'department'].includes(value)) return ['administrative_area_level_2'];
  if (['city', 'town', 'village', 'municipality', 'locality'].includes(value)) return ['locality'];
  if (['suburb', 'neighbourhood', 'quarter', 'hamlet', 'borough', 'city_block'].includes(value)) {
    return ['neighborhood'];
  }
  if (key === 'highway' || value === 'road') return ['route'];
  if (key === 'natural' || key === 'waterway' || value === 'natural') return ['natural_feature'];
  if (key === 'leisure' && value === 'park') return ['park'];
  if (key === 'aeroway' || value === 'aerodrome') return ['airport'];
  if (key === 'boundary' && value === 'administrative') return ['administrative_area_level_2'];
  return ['point_of_interest'];
}

/** Photon extent [minLon, maxLat, maxLon, minLat] → Google-style bounds. */
function extentToViewport(extent) {
...
/** Compose a readable label from Photon properties (name, city, country…). */
...
/**
 * Geocode `query` without any API key. Resolves to a Google-geocode-shaped
 * result ({formatted_address, types, geometry:{location, viewport}}) or null.
 * @param {string} query - free-text place query
 * @param {{lat: number, lon: number}|null} bias - view-centre bias, optional
 */
export async function keylessGeocode(query, bias = null) {
  // Photon first — typo-tolerant, CORS-open, built for app autocomplete loads.
  try {
    ...
    if (response.ok) {
      ...
      if (Number.isFinite(lat) && Number.isFinite(lng)) {
        ...
        return {
          formatted_address: photonLabel(props) || query,
          types: osmPlaceToGoogleTypes(props.osm_key, props.osm_value),
          geometry: {
            location: { lat, lng },
            viewport: extentToViewport(props.extent),
            bounds: extentToViewport(props.extent),
          },
        };
      }
    }
  ...
  // Nominatim backup — authoritative, rate-limited (1 req/s, fine for manual search).
  try {
    ...
    const viewport = box && box.every(Number.isFinite)
      ...
      : null;
    return {
      formatted_address: row.display_name || query,
      types: osmPlaceToGoogleTypes(row.class, row.addresstype || row.type),
      geometry: { location: { lat, lng }, viewport, bounds: viewport },
    };
  } catch {
    return null;
  }
...
/** Camera position as a {lat, lon} bias for the keyless geocoder, or null. */
function cameraBias(viewer) {
  const carto = viewer?.camera?.positionCartographic;
...
export async function searchAndFlyTo(viewer, query, options = {}) {
  ...
  if (apiKey) {
    ...
    // WATCHTOWER FREE mode — no Google key. Geocode through keyless OSM
    // services (Photon → Nominatim) instead of throwing; the result is shaped
    // like a Google geocode so everything below runs unchanged.
    result = await keylessGeocode(query, cameraBias(viewer));
  }
