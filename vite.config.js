...
function firmsProxy() {
  ...
  // WATCHTOWER — FREE fires fallback. Without a FIRMS_MAP_KEY the upstream
  // app answered 503 {error:'no_key'} and the layer showed a KEY badge. NASA
  // EONET (Earth Observatory Natural Event Tracker) lists OPEN wildfire
  // events with coordinates, needs NO key, and allows app traffic — so the
  // keyless path serves those in the same record shape instead. EONET events
  // are curated incidents (fewer, bigger fires than raw VIIRS detections);
  // acq date/time are stamped at fetch time because these are *currently
  // open* events, not 24h-windowed detections.
  const EONET_URL = 'https://eonet.gsfc.nasa.gov/api/v3/events?category=wildfires&status=open&limit=500';
  const EONET_TTL_MS = 30 * 60_000;
  /** @type {?{at: number, sources: Array<object>, fires: Array<object>}} */
  let eonetMem = null;
  /** @type {?Promise<?{at: number, sources: Array<object>, fires: Array<object>}>} */
  let eonetInflight = null;

  async function refreshEonet() {
    const res = await fetch(EONET_URL, { signal: AbortSignal.timeout(30_000) });
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
    const body = await res.json();
    const now = Date.now();
    const nowIso = new Date(now).toISOString();
    const acqDate = nowIso.slice(0, 10);
    const acqTime = `${nowIso.slice(11, 13)}${nowIso.slice(14, 16)}`;
    const fires = [];
    for (const event of body?.events || []) {
      const geometries = Array.isArray(event?.geometry) ? event.geometry : [];
      const point = [...geometries].reverse()
        .find((g) => g?.type === 'Point' && Array.isArray(g.coordinates));
      if (!point) continue;
    ...
    return {
      at: now,
      sources: [{ source: 'EONET_WILDFIRES_OPEN', count: fires.length, ok: true }],
      fires,
    };
  }

  function getEonet() {
    if (eonetMem && Date.now() - eonetMem.at < EONET_TTL_MS) {
      return Promise.resolve(eonetMem);
    }
    if (!eonetInflight) {
      eonetInflight = refreshEonet()
        .then((fresh) => { eonetMem = fresh; return fresh; })
        .catch((err) => {
          console.warn('[firms-proxy] EONET keyless fallback failed:', err?.message || err);
          return eonetMem; // stale-if-error, or null on a cold start
        })
    ...
    configureServer(server) {
      server.middlewares.use('/api/firms', async (req, res) => {
        ...
        try {
          ...
          if (!key) {
            // WATCHTOWER — FREE fires: serve keyless NASA EONET open-wildfire
            // events in the FIRMS payload shape; 503 no_key only when EONET
            // itself is unreachable and there is no cached entry.
            const entry = await getEonet();
            if (entry) {
              sendJson(200, {
                fetchedAt: entry.at,
                stale: Date.now() - entry.at >= EONET_TTL_MS,
                ttlMs: EONET_TTL_MS,
                sources: entry.sources,
...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    server: {
      ...
      // Set ALLOW_FRAMING=1 only for sandboxed/preview environments that must
      // embed the app in an iframe (e.g. a hosted live-preview proxy).
      headers: env.ALLOW_FRAMING === '1' ? {} : {
